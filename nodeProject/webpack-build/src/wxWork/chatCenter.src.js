        ...
        methods: {
                      ...
                      console.log(`发送成功`)
                        ...
                        Mt_Util.getCheckInfo().then(res => {
                            if (res && res.rt === 48002) { // 应用范围不可见
                                ...
                                this.$messagebox('提示', '已上线企微聊天工具栏自动登录功能请重新配置聊天工具栏后生效')
                            }
                        })
                    }
                })
                ...
                if (this.navSliderVal == 1) {
                    ...
                    Mt_Util.post('/ajax/wxWork/material/tsMaterial_h.jsp?cmd=getGroupMaterialList', params).then((res) => {
                        if (res.data && res.data.success) {
                            ...
                            res.data.data.forEach(item => { // 遍历分配话术到对应分组
                                ...
                                if (this.navSliderVal == 1) { 
                                    ...
                                    item.showText = item.showText.replace(new RegExp(searchName, 'g'), `<span class="keyword">${searchName}</span>`);
                                }
                                groupInfo.materialList.push(item);
                            });
    
                            let hasMaterialList = tempList.filter(item => item.materialList.length > 0 || tempList.filter(subItem => item.id != 0 && subItem.parentId == item.id).filter(childItem => childItem.materialList.length > 0).length > 0);
                            let groupTagParentList = hasMaterialList.filter(item => item.parentId == 0); // 筛选出所有一级分组
                            groupTagParentList.forEach(parent => {
                                parent.childGroupList = parent.id == 0 ? [] : hasMaterialList.filter(child => child.parentId == parent.id); // 将所有二级分组归类到对应的一级分组
                            });
            ...
            getGroupTagList() {
                ...
                Mt_Util.post('/ajax/comm/tsGroup_h.jsp?cmd=getTsGroupList', params).then((res) => {
                    if (res.data && res.data.success) {
                        ...
                        groupTagParentList.forEach(parent => {
                            parent.childGroupList = parent.id == 0 ? [] : res.data.data.filter(child => child.parentId == parent.id); // 将所有二级分组归类到对应的一级分组
                        });
            ...
            */
            setMaterialListByGroupId(groupId, materialList) {
                materialList.forEach(item => { // 因为话术的展示用了v-html来实现搜索时关键词高亮，所以在设置对应分组下的话术列表时，需要encode一下以防XSS
                    ...
                    if (!this.filePath.length) {
                        if (this.pageStatus == 'normal') {
                            this.filePath = [{
                                ...
                                name: this.normalSliderVal == 1 ? '企业文件' : this.normalSliderVal == 3 ? '产品文件' : '我的文件'
                            }] 
            ...
            getFileList() {
                ...
                return new Promise((resolve) => {  
                    Mt_Util.post(url, 
                        Object.assign({}, this.fileRequestParam, this.otherCheckOption)
                    ).then((res) => {
                        if (res.data && res.data.success) {
