...
Mt_Util.router.addFollow = Vue.extend({
    ...
    created(){
        ...
        //时间默认选择当前
        this.followInfo.date = Ts_util.format.formatDate(Ts_util.format.addDate(new Date(), 1, "days")).split(' ')[0]
        this.followInfo.time = Ts_util.format.formatDate(Ts_util.format.addDate(new Date(), 1, "days")).split(' ')[1].substring(0, 5)
    },
    methods: {
        /**
        * 初始化弹窗选择内容
        ...
        */
        ...
        chooseNextTime(days,index){
            ...
            if (days!==0){
                this.followInfo.date = Ts_util.format.formatDate(Ts_util.format.addDate(new Date(), days, "days")).split(' ')[0]
                this.followInfo.time = Ts_util.format.formatDate(Ts_util.format.addDate(new Date(), 1, "days")).split(' ')[1].substring(0, 5)
            }else{
        ...
        */
        ...
        sureFollow() {
            ...
            //     return
            // }
            this.followInfo.visitsTime = this.followInfo.date?this.followInfo.date + ' ' + this.followInfo.time + ':00':''
            Mt_Util.post("/ajax/client/tsClientVisits_h.jsp?cmd=addTsClientVisits", this.followInfo).then(res => {
                if (res.data && res.data.success) {
                    window.history.go(-1);
                    this.$parent.initFollowList()
                    this.$parent.getCustomInfo()
                } else if (res.data && res.data.rt === 6004) {//版本提醒弹窗
