(function () {
    ...
    `,
        ...
        async created() {
            ...
            Mt_Util.initWxConfig().then(() => {
                Mt_Util.checkIsOutGroup().then((res) => {
                    if(!res) {
                        this.getCurExternalContact().then(externalUserId => { // 获取userid
        ...
        computed: {
             ...
             */
            isShowAction(){
                return this.sliderVal == 0 && (this.viewerList.length <=1 || this.isInDetail || this.bindViewerId !== 0);
            },
            /**
            ...
            */
            showEmpty() {
                return !this.isCheckedApplication || !this.hasCustomer || !this.isProfVer || !this.hasFuncPermission
            },
            /**
             * 是否显示缺省页按钮
             * @author guoyijie
             ...
             */
            showEmptyBtn() {
                return (!this.isCheckedApplication || !this.isSyncCustomerInfo || !this.isProfVer) && this.hasFuncPermission
            },
            /**
             * 缺省页按钮文案
             * @author guoyijie
        ...
        methods: {
                ...
                return new Promise(resolve=>{
                    ...
                    Mt_Util.post("/rest/manage/client/getTsClientByWxWork", params).then(res => {
                        if (res.data && res.data.success) {
                            ...
                            let otherKeyList = ['corpName', 'industry', 'position', 'area', 'address', 'remark','customersWay']
                            ...
                            if (wxWorkRemark !== undefined) {
                                ...
                                if (wxWorkRemark.mobiles != undefined && wxWorkRemark.mobiles.length > 0) {
                                    ...
                                    this.headerInfo.mobiles = this.headerInfo.mobiles.substring(0, this.headerInfo.mobiles.length - 1)
                                }  
                            }
                            ...
                            this.customInfoList.forEach(item => {
                                item.isAble = systemFieldAbleConf[item.key] !== undefined ? systemFieldAbleConf[item.key]:true;
                            ...
                            this.otherInfoList.forEach(item => {
                                ...
                                if (customerData[item.key] !== undefined) {
                                    ...
                                    } else if (item.key === 'province') {//区域=省+市
                            ...
                            this.saleInfoList.forEach(item => {
                                if (customerData[item.key] !== undefined) {
                                    item.value = item.key == 'salesRelTime' ? Ts_util.format.formatDate(customerData[item.key], "yyyy-MM-dd HH:mm:ss") : customerData[item.key];
                                    
                                }
                            });
                            
                            //企微信息
                            wxWorkRemark && this.qwInfoList.forEach(item => {
            ...
            * */
            getViewerList() {
                return new Promise(resolve => {
                    ...
                    Mt_Util.post("/ajax/wxWork/viewer/tsViewer_h.jsp?cmd=getViewerList", params).then(res => {
                        if (res.data && res.data.success) {
            ...
            bindViewerById(id) {
                return new Promise(resolve => {
                    this.$messagebox.confirm('', {
                        ...
                        Mt_Util.post('/ajax/wxWork/client/tsClient_h.jsp?cmd=bindViewer', params).then(res => {
                            if (res.data && res.data.success) {
                                this.$messagebox({
                                    title: '关联成功',
                                    message: '已成功关联该访客',
                                    ...
                                    if (!this.isInDetail) { // 如果本身就是留在详情页面的，就不重新查询了
            ...
            * */
            unBindViewer() {
                ...
                this.$messagebox.confirm('', {
                    ...
                    Mt_Util.post('/ajax/wxWork/client/tsClient_h.jsp?cmd=unBindViewer', params).then(res => {
                        if (res.data && res.data.success) {
            ...
            */
            seeNextViewer() {
                if (this.viewerList.length) {
                    ...
                    currentIndex++;
                    if (currentIndex === this.viewerList.length) { // 后端默认id排序，-1代表当前绑定的为同名访客的最后一个，需要从头开始
                        this.currentViewerId = this.viewerList[0].id
                    } else {
            ...
            */
            getActionList() {
                ...
                return new Promise(resolve => {
                    Mt_Util.post('/ajax/wxWork/viewer/tsViewerRecord_h.jsp?cmd=getRecordList', this.actionParams).then(res => {
                        if (res.data && res.data.success) {
            ...
            getFollowList() {
                ...
                return new Promise(resolve => {
                    Mt_Util.post('/ajax/client/tsClientVisits_h.jsp?cmd=getTsClientVisitsList', parmes).then(res => {
                        if (res.data && res.data.success) {
                            res.data.data.forEach(item => {
                                item.date = Ts_util.format.date(new Date(item.date), 'yyyy-MM-dd')
                                if (item.list && item.list.length > 0) {
                                    item.list.forEach(subItem => {
                                        subItem.createTimeName = subItem.createTime ? Ts_util.format.date(new Date(subItem.createTime), 'HH:mm') : ''
                                        subItem.visitTimeName = subItem.visitsTime ? Ts_util.format.date(new Date(subItem.visitsTime), "yyyy-MM-dd HH:mm") : ""
                                    })
                                }
                            })
                            this.actionParams.maxPage = Math.ceil(res.data.total / this.actionParams.limit)
            ...
            */
            setStatus(data, isEnd, index) {
                if (isEnd) {
                    Mt_Util.FdpLog('yx_gjjdjg', { // 跟进阶段结果
                        yx_free_text_0: `点击${index == 0 ? '成交' : '未成交'}`,
                        yx_app_terminal: 4
                    });
                } else {
                    Mt_Util.FdpLog('yx_gjjd', { // 跟进阶段结果
                ...
                this.$messagebox.confirm('', {
                    ...
                    Mt_Util.post('/ajax/client/tsClient_h.jsp?cmd=setTsClientFollowStatus', params).then((res) => {
                        if (res.data && res.data.success) {
            ...
            */
            setInitStatus(status) {
                if (status == 0) {
                    ...
                    let currentItem = this.followStatusList.find(item => { // 判断当前所处阶段是否被删除
            ...
            cancelAssist() {
                ...
                this.$messagebox.confirm('', {
                    ...
                    Mt_Util.post('/ajax/wxWork/client/tsClient_h.jsp?cmd=unbindAssistClient', { cid: this.customerId }).then(res => {
