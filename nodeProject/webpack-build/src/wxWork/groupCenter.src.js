;(() => {
  ...
  Mt_Util.router.groupCenter = Vue.extend({
    ...
    computed: {
      ...
      groupTypeText() {
        let groupTypeObj = this.groupTypeList.find(
          (item) => item.value === this.requestParams.isMyClient
        )
        ...
        return new Promise((resolve) => {
          ...
          Mt_Util.post('/rest/manage/wxwork/chatMember/getChatMemberList', parmes).then((res) => {
            if (res.data && res.data.success) {
              ...
              this.requestParams.maxPage = Math.ceil(res.data.total / this.requestParams.limit)
              resolve(memberList)
            } else {
              this.showEmpty = true
            }
