  ...
  Mt_Util.router.setAssist = Vue.extend({
    ...
    created() {
      ...
      this.getDepartmentInfo([this.checkOption.id]).then((data) => {
        ...
        this.allDepIdArr = data.depIdList.filter((id) => id != this.checkOption.id) // 过滤掉父级部门id
      })
    },
    mounted() {
      debugger
    ...
    methods: {
      ...
      checkParent() {
        ...
        let allSidFlag =
          new Set([...currentSidArr, ...this.allSidArr]).size === currentSidArr.length
        let allDepFlag =
          new Set([...currentDepIdArr, ...this.allDepIdArr]).size === currentDepIdArr.length
        if (allSidFlag && allDepFlag) {
          currentDepIdArr.push(this.checkOption.id)
        } else {
          currentDepIdArr = currentDepIdArr.filter((id) => id != this.checkOption.id)
        }
      ...
      saveReceiver() {
        ...
        if (len >= 2) {
          timeInfo.commit(
            ...
            Object.assign({}, timeInfo.state.selectPages[len - 2])
          )
