;(function () {
  ...
  Mt_Util.router.editCustom = Vue.extend({
    ...
    activated() {
      this.customInfo.assistSidList = timeInfo.state.fromAdd
        ? timeInfo.state.tempSidArr
        : this.customInfo.assistSidList
    ...
    methods: {
      ...
      toAssist() {
        let arr =
          timeInfo.state.tempSidArr.length && timeInfo.state.fromAdd
            ? [...timeInfo.state.tempSidArr]
            : [...this.customInfo.assistSidList]
        debugger
        ...
        return new Promise((resolve) => {
          ...
          Mt_Util.post('/rest/manage/client/getTsClientByWxWork', params).then((res) => {
            if (res.data && res.data.success) {
              ...
              this.customInfo.industry =
                this.customInfo.industry == 0 ? '' : this.customInfo.industry
              this.customInfo.customersWay =
                this.customInfo.customersWay == 0 ? '' : this.customInfo.customersWay
              this.customInfo.birthday = this.customInfo.birthday
                ? Ts_util.format.formatDate(customData.birthday).split(' ')[0]
                : this.customInfo.birthday
              ...
              this.customInfo.assistSidList = timeInfo.state.fromAdd
                ? timeInfo.state.tempSidArr
                : customData.assistInfo.assistSidList
              !timeInfo.state.sidArr.length &&
                (timeInfo.state.sidArr = this.customInfo.assistSidList)
              // this.customInfo.assistNames = timeInfo.state.fromAdd?timeInfo.state.assistNames:customData.assistInfo.assistNames
              this.assistNames = timeInfo.state.fromAdd
                ? timeInfo.state.assistNames
                : customData.assistInfo.assistNames
              ...
              this.remarkInfo.remarkMobiles &&
                this.remarkInfo.remarkMobiles.forEach((item, index) => {
          ...
          switch (this.selectType) {
            ...
            case 3: //获客方式
              this.customInfo.customersWay = this.selectValue
                ...
                ? this.customInfo.customersWay
                : 1
              this.customInfo.customersWayName = this.customersWayList[
                this.customInfo.customersWay - 1
              ]
              break
          }
        }
      ...
      sureEdit() {
        ...
        this.customInfo.customersWay = this.customInfo.customersWay
          ? this.customInfo.customersWay
          : 0
        this.customInfo.crmFieldDataList = JSON.stringify(
          this.personFieldsList.concat(this.otherFieldsList)
        )
        // this.customInfo.assistSidList = JSON.stringify(this.customInfo.assistSidList)
        this.customInfo.assistSidList = timeInfo.state.fromAdd
          ? JSON.stringify(timeInfo.state.tempSidArr)
          : JSON.stringify(this.customInfo.assistSidList)
        this.remarkInfo.remarkMobiles = JSON.stringify(mobileList)
        let parmes = Object.assign({}, this.customInfo, this.remarkInfo, {
          isPc: false,
          sid: timeInfo.state.staffInfo.mysid
        })
