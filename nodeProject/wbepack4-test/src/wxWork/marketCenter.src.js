    ...
    methods: {
      ...
      getArticleListByTitle(title) {
        ...
        if (this.articleSliderVal !== 0) {
          this.articleClassifyList[this.articleSliderVal]
            ? (this.checkOption.type = this.articleClassifyList[this.articleSliderVal][0].id)
            : -1
        }
      ...
      getTsArticleRouterInfo() {
        return new Promise((resolve) => {
          Mt_Util.post('/ajax/wxWork/article/tsArticle_h.jsp?cmd=getTsArticleRouterInfo').then(
            (res) => {
              if (res.data && res.data.success) {
      ...
      getMarketInfo() {
        return new Promise((resolve) => {
          ...
          let data = Object.assign({}, this.checkOption, this.otherCheckOption)
