...
const dutyPath = path.resolve(__dirname, '../models/mock/dutyData.json')
fs.readJson(dutyPath, (err, res) => {
	console.log(`err`, err, dutyPath)
	if (!err) {
		const dutyData = { ...res }
...
const taskSchedule = function (dutyData) {
	...
	if (
		Array.isArray(dutyData.currentDutyList) &&
		dutyData.currentDutyList.length
	) {
		dutyData.currentDutyList.forEach(item => {
			hooksData.dutyIndex_web = dutyData.memberList_web.findIndex(
	...
	if (new Date().getHours() > 9) {
		// 超过当天临界时间的，则放入第二天的数据
		// 超过遍历人数的则重头开始
		hooksData.dutyIndex_web =
			hooksData.dutyIndex_web >= dutyData.memberList_web.length - 1
				? 0
				: hooksData.dutyIndex_web + 1
	}
			...
			if (!err) {
				...
				hooksData.dutyIndex_web =
					hooksData.dutyIndex_web >= dutyData.memberList_web.length - 1
						...
						: hooksData.dutyIndex_web + 1
				console.log(
					`写入${currentDuty_web.name}，${new Date().toLocaleDateString()}`
				)
			} else {
